...
public class ConfigHelper {
	public enum Transform {
		NUMBER, SWITCH, STRING, ROLLERSHUTTER, DATETIME, TIME, CONTACT, DIMMER, COLOR
	}
	...
	public ConfigHelper(Pages pages, CometVisuApp app, String sitemapName) {
		...
		logger.info("icon mapping enabled: "
				+ Config.iconConfig
						.get(Config.COMETVISU_ICON_ENABLE_MAPPING_PROPERTY)
	...
	private void initStatusBar() {
		...
		confStatus
				.setValue("<img src=\"icon/comet_64_ff8000.png\" alt=\"CometVisu\" /> by <a href=\"http://www.cometvisu.org/\">CometVisu.org</a>");
		...
		String version = "autogenerated from openHAB " + sitemapName
				+ " sitemap";
		checkStatus.setValue("- <a href=\"check_config.php\">Check Config</a>"
				+ "<div style=\"float:right;padding-right:0.5em\">Version: "
				+ version + "</div>");
	...
	public boolean hasIconMapping(String ohIconName) {
		return Config.iconConfig
				.get(Config.COMETVISU_ICON_ENABLE_MAPPING_PROPERTY) != null
				&& Config.iconConfig.get(
						Config.COMETVISU_ICON_ENABLE_MAPPING_PROPERTY).equals(
						"true") && Config.iconMappings.get(ohIconName) != null;
	}
	...
	public Label addLabel(Object element, String name, String iconName) {
		if (element == null)
			return null;
		try {
			Method setter = element.getClass().getMethod("setLabel",
					Label.class);
			if (setter != null) {
	...
	public Mapping addMapping(Object element, Mapping mapping) {
		if (element != null && mapping != null) {
			try {
				Method setter = element.getClass().getMethod("setMapping",
						String.class);
	...
	public void mapToTriggers(Object element, Item item, Widget widget) {
		...
		int triggerColumns = Math.max(1, Math
				.round((groupColumns - textColumns) / sitemapMapping.size()));
...
//		addToMappings(mapping);
	...
	private EList<org.eclipse.smarthome.model.sitemap.Mapping> getMapping(
			Widget widget) {
		EList<org.eclipse.smarthome.model.sitemap.Mapping> mapping = null;
		try {
			Method getter = widget.getClass().getMethod("getMappings");
			mapping = (EList<org.eclipse.smarthome.model.sitemap.Mapping>) getter
					.invoke(widget);
		} catch (NoSuchMethodException | SecurityException e) {
	...
	private EList<org.eclipse.smarthome.model.sitemap.ColorArray> getStyling(
			Widget widget) {
		EList<org.eclipse.smarthome.model.sitemap.ColorArray> styling = null;
		try {
			Method getter = widget.getClass().getMethod("getValueColor");
			styling = (EList<org.eclipse.smarthome.model.sitemap.ColorArray>) getter
					.invoke(widget);
		} catch (NoSuchMethodException | SecurityException e) {
			logger.error(e.getMessage());
		} catch (IllegalAccessException e) {
	 ...
	 *            navbar)
	 */
	public void addSeparatorToNavbar(Page page, NavbarPositionType position,
			boolean ifNotEmpty) {
		...
		if (navbar != null) {
			if (!ifNotEmpty || navbar.getPageOrGroupOrLine().size() > 0) {
				...
				navbar.getPageOrGroupOrLine().add(
	 ...
	 * @param widget
	 */
	public void addToNavbar(Page barPage, Page targetPage,
			org.eclipse.smarthome.model.sitemap.Group widget,
		...
		if (item != null && item.getLabel()!=null) {
			...
			if (m.matches()) {
				...
				if (format.contains("%d")) {
					...
					Address address = new Address();
					address.setTransform("OH:"
							+ Transform.NUMBER.toString().toLowerCase());
					address.setValue(Transform.NUMBER.toString().toLowerCase()
							+ ":" + item.getName());
	...
	public void addIconDefinition(IconDefinition iconDef) {
		...
		logger.trace("adding icon-def '{}' url '{}'", iconDef.getName(),
				iconDef.getUri());
	}
	...
	private Navbar getNavbar(Page page, NavbarPositionType position) {
		...
		for (JAXBElement<?> elem : page.getPageOrGroupOrNavbar()) {
			if (elem.getValue() instanceof Navbar
					&& ((Navbar) elem.getValue()).getPosition()
							.equals(position)) {
	 ...
	 */
	private void addToNavbar(Page page, Pagejump pagejump,
			NavbarPositionType position) {
		Navbar navbar = getNavbar(page, position);
		if (navbar == null) {
		...
		navbar.getPageOrGroupOrLine().add(
				factory.createNavbarPagejump(pagejump));
	}
...
//				String mapFile = m.group(1);
				format = m.group(2);
				method.invoke(elem,format);
//				if (mapFile!=null && !mapFile.isEmpty()) {
//					Mapping mapping = initMappingFromFile(mapFile);
//					addMapping(elem,mapping);
//				}
			}
		} catch (NoSuchMethodException | SecurityException e) {
			logger.error(e.getMessage());
	...
	public void cleanup(Object page, Pages pages) {
		List<JAXBElement<?>> children = null;
		...
		for (JAXBElement<?> element : children) {
			if (element.getValue() instanceof Page) {
				...
				if (visible == 0 && firstChildPage != null) {
					...
					for (JAXBElement<?> e : pages.getPage()
							.getPageOrGroupOrNavbar()) {
						if (e.getValue() instanceof Navbar) {
							Navbar navbar = (Navbar) e.getValue();
							for (JAXBElement<?> ne : navbar
									.getPageOrGroupOrLine()) {
				...
				if (visible == 0) {
					if (logger.isTraceEnabled())
						logger.trace("group '{}' has no visible elements",
								group.getName());
		...
		if (childsToAdd.size() > 0 && page instanceof Page)
			if (logger.isTraceEnabled())
				logger.trace("there are '{}' children to be added to '{}'",
						childsToAdd.size(), ((Page) page).getName());
		...
		for (JAXBElement<?> element : groupsToDelete) {
			if (page instanceof Page) {
				if (logger.isTraceEnabled())
					logger.trace("removing group '{}' from '{}'", element,
							((Page) page).getName());
