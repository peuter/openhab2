 ...
 *
 */
@Path(Config.COMETVISU_BACKEND_ALIAS + "/"
		+ Config.COMETVISU_BACKEND_CHART_ALIAS)
public class ChartResource {
	private static final Logger logger = LoggerFactory
			.getLogger(ChartResource.class);
	...
	protected final static String RRD_FOLDER = getUserDataFolder()
	...
	public Response getChartSeries(@Context HttpHeaders headers,
		...
		try {
			...
			// Prefer RRD-Service
			QueryablePersistenceService persistenceService = getPersistenceServices()
					.get(service);
			// Fallback to first persistenceService from list
			if (persistenceService == null) {
				Iterator<Entry<String, QueryablePersistenceService>> pit = getPersistenceServices()
						.entrySet().iterator();
				if (pit.hasNext()) {
			...
			if (persistenceService.getName().equals("rrd4j")) {
				data = getRrdSeries(persistenceService, item,
						consilidationFunction, startTime, endTime, resolution);
			} else {
	...
	public Object getPersistenceSeries(
		...
		while (it.hasNext()) {
			...
			org.eclipse.smarthome.core.types.State state = historicItem
					.getState();
