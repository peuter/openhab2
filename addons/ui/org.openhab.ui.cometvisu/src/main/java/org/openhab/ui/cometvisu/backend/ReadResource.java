...
@Path(Config.COMETVISU_BACKEND_ALIAS + "/"
		+ Config.COMETVISU_BACKEND_READ_ALIAS)
public class ReadResource implements EventBroadcaster {
	...
	public Object getStates(@QueryParam("a") List<String> itemNames,
		...
		if (this.itemRegistry != null) {
			...
			logger.debug("initially broadcasting " + states.size() + "/"
					+ itemNames.size() + " item states");
	...
	public void registerItem(Item item) {
		if (item == null || items.containsKey(item)
				|| !itemNames.contains(item.getName()))
			return;
		if (item instanceof GenericItem) {
			((GenericItem) item).addStateChangeListener(stateEventListener);
		}
