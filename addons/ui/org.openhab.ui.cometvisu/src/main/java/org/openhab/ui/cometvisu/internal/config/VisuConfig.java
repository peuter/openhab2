...
import org.openhab.ui.cometvisu.internal.config.beans.*;
...
public class VisuConfig {
	...
	public String getConfigXml(HttpServletRequest req) {
		...
		return marshal(pagesBean, rootFolder.getAbsolutePath() + File.separator
				+ schemaFile);
	}
	 ...
	 */
	private void processWidget(Object rootPage, Widget widget, Pages pages,
			int level) {
		Item item = null;
		...
		if (widget instanceof LinkableWidget) {
			EList<Widget> children = app.getItemUIRegistry().getChildren(
			...
			if (children.size() == 0) {
				...
				configHelper
						.addToRoot(rootPage, factory.createPageGroup(group));
				...
				if (widget instanceof org.eclipse.smarthome.model.sitemap.Group) {
					...
					NavbarPositionType position = (level <= 1) ? NavbarPositionType.TOP
							: NavbarPositionType.LEFT;
					Page barPage = (rootPage instanceof Page) ? (Page) rootPage
							: pages.getPage();
					...
					configHelper.addToNavbar(barPage, page, group, position,
							item);
	...
	private void processItemWidget(Object rootPage, Widget widget, Item item,
			Pages pages, int level) {
		if (widget instanceof org.eclipse.smarthome.model.sitemap.Switch) {
			...
			if (item instanceof RollershutterItem) {
				...
				configHelper
						.addLabel(switchBean, configHelper.getLabel(widget));

				configHelper.addMapping(switchBean, widget);
				configHelper.addStyling(switchBean, widget);
				configHelper.addToRoot(rootPage,
						factory.createPageSwitch(switchBean));
			}
			...
			String mappingName = String.valueOf(selection.getMappings()
					.hashCode());
			Mapping mapping = configHelper.createMapping(mappingName,
					selection.getMappings());
			configHelper.addToMappings(mapping);

			for (org.eclipse.smarthome.model.sitemap.Mapping map : selection
					.getMappings()) {
				Trigger trigger = new Trigger();
				trigger.setValue(map.getCmd());
				trigger.setMapping(mappingName);
				trigger.getAddress().add(address);
				trigger.setLayout(configHelper.createLayout(Math.max(1,
						6 / selection.getMappings().size())));

				configHelper
						.addToRoot(bean, factory.createPageTrigger(trigger));
			}
		...
		} else if (widget instanceof org.eclipse.smarthome.model.sitemap.Chart
				&& item != null) {
			...
			if (item instanceof GroupItem) {
				for (Item member : ((GroupItem) item).getMembers()) {
					...
					if (member instanceof NumberItem) {
						rrd.setConsolidationFunction(ConsolFun.AVERAGE
								.toString());
					} else {
			...
			configHelper.addToRoot(rootPage,
					factory.createPageColorchooser(bean));
		} else {
