...
@Path(Config.COMETVISU_BACKEND_ALIAS + "/"
		+ Config.COMETVISU_BACKEND_WRITE_ALIAS)
public class WriteResource {
	private static final Logger logger = LoggerFactory
			.getLogger(WriteResource.class);
	...
	public Response setState(@Context HttpHeaders headers,
			@QueryParam("a") String itemName, @QueryParam("v") String value,
			@QueryParam("ts") long timestamp) {
		if (logger.isDebugEnabled())
			logger.debug(
					"Received CV write request at '{}' for item '{}' with value '{}'.",
					uriInfo.getPath(), itemName, value);
		Item item;
		try {
