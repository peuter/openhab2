...
public class CometVisuServlet extends HttpServlet {
	...
	private static final Logger logger = LoggerFactory
			.getLogger(CometVisuServlet.class);
	...
	private DateFormat rssPubDateFormat = new SimpleDateFormat(
	...
	public CometVisuServlet(String filesystemDir, CometVisuApp cometVisuApp) {
		...
		engine.getQuercus().setIni("include_path",
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp)
		...
		if (!requestedFile.exists() && matcher.find()) {
			...
			if (sitemap != null) {
				...
				throw new ServletException("Sitemap '" + matcher.group(1)
						+ "' could not be found");
			}
		}
	 ...
	 * @param response
	 */
	private void processRssLogRequest(File file, HttpServletRequest request,
		...
		if (items.size() > 0) {
			...
			if (!cometVisuApp.getPersistenceServices().entrySet().iterator()
					.hasNext()) {
			...
			if (request.getParameter("c") != null) {
				if (items.size() == 1) {
					...
					// Build message
					Command command = new StringType(title
							+ rssLogMessageSeparator + message
							+ rssLogMessageSeparator + state
				...
				response.setContentType("text/plain");
				response.getWriter()
						.write("Cannot execute query: It is not possible to delete data from openHAB PersistenceService");
				response.flushBuffer();
			} else if (request.getParameter("u") != null) {
				// update state
				response.setContentType("text/plain");
				response.getWriter()
						.write("Cannot execute query: It is not possible to update data from openHAB PersistenceService");
				response.flushBuffer();
...
//				filter.setEndDate(end);
				...
				for (Item item : items) {
					...
					Iterator<Entry<String, QueryablePersistenceService>> pit = cometVisuApp
							.getPersistenceServices().entrySet().iterator();
					QueryablePersistenceService persistenceService = pit.next()
							.getValue();
					// Get the data from the persistence store
					Iterable<HistoricItem> result = persistenceService
							.query(filter);
					...
					if (it.hasNext()) {
						logger.debug("persisted data for item {} found in service {}",item.getName(),persistenceService.getName());
					}
					...
					while (it.hasNext()) {
						...
						entry.publishedDate = historicItem.getTimestamp()
								.getTime();
						logger.info(rssPubDateFormat.format(entry.publishedDate)+": "+historicItem.getState());
						entry.tags = historicItem.getName();
						String[] content = historicItem.getState().toString()
								.split(rssLogMessageSeparator);
						if (content.length == 0) {
					...
					if ("rrd4j".equals(persistenceService.getName()) && FilterCriteria.Ordering.DESCENDING.equals(filter.getOrdering())) {
						// the RRD4j PersistenceService does not support descending ordering so we do it manually
						Collections.sort(feed.entries, new Comparator<org.openhab.ui.cometvisu.internal.rrs.beans.Entry>() {
							@Override
							public int compare(
									org.openhab.ui.cometvisu.internal.rrs.beans.Entry o1,
					...
					logger.debug(
							"querying {} item from {} to {} => {} results on service {}",
							filter.getItemName(), filter.getBeginDate(),
							filter.getEndDate(), i,
				...
				if (request.getParameter("j") != null) {
					...
					for (org.openhab.ui.cometvisu.internal.rrs.beans.Entry entry : feed.entries) {
						...
						rss += "<description>" + entry.content
								+ "</description>";
						Date pubDate = new Date(entry.publishedDate);
						rss += "<pubDate>" + rssPubDateFormat.format(pubDate)
								+ "</pubDate>";
						rss += "</item>\n";
					}
	 ...
	 *       .html
	 */
	private void processStaticRequest(File file, HttpServletRequest request,
		...
		if (file == null) {
			...
			// prepare
			// file object.
			file = new File(rootFolder, URLDecoder.decode(requestedFile,
					"UTF-8"));
		}
	 ...
	 * @throws IOException
	 */
	private final void dataProviderService(File file,
		...
		if (file.getName().equals("dpt_list.json")) {
			...
			// all item names
			File iconDir = new File(rootFolder,
					"icon/knx-uf-iconset/128x128_white/");
			FilenameFilter filter = new FilenameFilter() {
				@Override
				public boolean accept(File dir, String name) {
	 ...
	 * Service.
	 */
	private final void phpService(File file, HttpServletRequest request,
			HttpServletResponse response) throws ServletException, IOException {
		Env env = null;
		WriteStream ws = null;

		QuercusHttpServletRequest req = new QuercusHttpServletRequestImpl(
				request);
		QuercusHttpServletResponse res = new QuercusHttpServletResponseImpl(
				response);
		...
		try {
			...
			logger.info("setting user dir to "
					+ path.getParent().getNativePath());
			System.setProperty("user.dir", path.getParent().getNativePath());
			quercus.setServletContext(new QuercusServletContextImpl(
					_servletContext));
			...
			try {
				...
				StringValue prepend = quercus.getIniValue("auto_prepend_file")
						.toStringValue(env);
				if (prepend.length() > 0) {
					Path prependPath = env.lookup(prepend);

					if (prependPath == null)
						env.error(L.l("auto_prepend_file '{0}' not found.",
								prepend));
					else {
						QuercusPage prependPage = engine.getQuercus().parse(
								prependPath);
				...
				StringValue append = quercus.getIniValue("auto_append_file")
						.toStringValue(env);
				if (append.length() > 0) {
					Path appendPath = env.lookup(append);

					if (appendPath == null)
						env.error(L.l("auto_append_file '{0}' not found.",
								append));
					else {
						QuercusPage appendPage = engine.getQuercus().parse(
								appendPath);
				...
				RuntimeException myException = new RuntimeException(L.l(
						"StackOverflowError at {0}", env.getLocation()), e);
	...
	protected Path getPath(File file, QuercusHttpServletRequest req) {
		...
		logger.info(
				"ServletPath '{}', PathInfo: '{}', ScriptPath: '{}' => Path '{}'",
				servletPath, pathInfo, scriptPath, path);
